...
object Migration {
   ...
   */
  case class SchemaDiff(added: PropertyList, modified: PropertyList, removed: Set[JsonPointer])
  ...
  def diffMaps(source: PropertyList, successive: List[PropertyList]): SchemaDiff = {
    ...
    val removedKeys = (source.map(_._1) diff target.map(_._1)).toSet
   ...
   */
  def getAddedKeys(source: PropertyList, successive: List[PropertyList]): List[JsonPointer] = {
    val (newKeys, _) = successive.foldLeft((List.empty[JsonPointer], source)) { case ((acc, previous), current) =>
   ...
   */
  def getModifiedProperties(source: PropertyList, target: PropertyList, addedKeys: List[JsonPointer]): PropertyList = {
    val targetModified = target.filter { case (k, _) => !addedKeys.contains(k) }
    List(targetModified diff source: _*)
  }

  /**
   * Get `ListMap` ordered by key
   *
   * @param keys ordered list of submap keys
   * @param original original Map
   * @return sorted Map of new properties
   */
  def getSubmap[K, V](keys: List[JsonPointer], original: PropertyList): List[(JsonPointer, Schema)] =
   ...
   */
  def getOrdering(migrationMap: ValidMigrationMap): Map[RevisionGroup, Validated[String, List[JsonPointer]]] =
