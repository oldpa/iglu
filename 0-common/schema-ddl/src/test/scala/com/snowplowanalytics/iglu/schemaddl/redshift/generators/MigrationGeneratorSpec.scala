...
class MigrationGeneratorSpec extends Specification { def is = s2"""
  ...
  val empty = List.empty[(JsonPointer, Schema)]

  def e1 = {
    val diff = Migration.SchemaDiff(List("status".jsonPointer -> json"""{"type": "string"}""".schema), empty, Set.empty[JsonPointer])
  ...
  def e2 = {
    val diff = Migration.SchemaDiff(empty, empty, Set.empty[JsonPointer])
  ...
  def e3 = {
    ...
    val diff = Migration.SchemaDiff(newProps, empty, Set.empty[JsonPointer])
