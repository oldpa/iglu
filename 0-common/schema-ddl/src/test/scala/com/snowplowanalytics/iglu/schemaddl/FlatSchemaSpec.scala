...
import com.snowplowanalytics.iglu.schemaddl.jsonschema.{ Schema, JsonPointer }
...
class FlatSchemaSpec extends Specification { def is = s2"""
  ...
  def e1 = {
    ...
    val expected = FlatSchema(List(JsonPointer.Root -> Schema.empty.copy(`type` = Some(Type.Object)) ), Set.empty)
  ...
  def e3 = {
    ...
    val expected = FlatSchema(List(
      JsonPointer.Root -> Schema.empty.copy(description = Some(Description(description))) ), Set.empty)
  ...
  def bePointers(expected: Set[JsonPointer]): Matcher[Set[JsonPointer]] = { actual: Set[JsonPointer] =>
